...
// src/app/reports/questions/page.tsx
'use client';
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {reports.map((question, index) => (
                  <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-green-600 dark:text-green-400">
                      {question.avg_student_score?.toFixed(1)}%
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                    {reports.reduce((sum, r) => sum + (r.total_usage || 0), 0)}
                  </p>
            ...
            {reportType === 'by_difficulty' && (
              <>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                    {(reports.reduce((sum, r) => sum + (r.avg_marks || 0), 0) / reports.length).toFixed(1)}
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Avg Marks</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                    {(reports.reduce((sum, r) => sum + (r.avg_usage || 0), 0) / reports.length).toFixed(1)}
                  </p>
            ...
            {reportType === 'usage_stats' && (
              <>
                <div className="text-center">
                  <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                    {reports.reduce((sum, r) => sum + (r.usage_count || 0), 0)}
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Total Usage</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                    {(reports.reduce((sum, r) => sum + (r.avg_student_score || 0), 0) / reports.length).toFixed(1)}%
                  </p>
