...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create paper and assign programmes
    const paperResult = await query<{insertId: number}>(
      `INSERT INTO exam_papers (
        paper_code, course_id, created_by, exam_type,
        academic_year, semester, exam_date, duration,
        instructions, status, hod_id, dean_id
      ) VALUES (?, ?::int, ?::int, ?::text::exam_type, ?::int, ?::int, ?::date, ?::int, ?, 'draft', ?::int, ?::int)`,
      [
        paper_code,
        course_id,
        session.id,
    ...
    if (programme_ids.length > 0) {
      for (const progId of programme_ids) {
        await query(
          `INSERT INTO exam_paper_programmes (exam_paper_id, programme_id) VALUES (?::int, ?::int)`,
          [paper_id, progId]
        );
      }
    }
    ...
    await query(
      `INSERT INTO workflow_history (exam_paper_id, action, from_status, to_status, actor_id, actor_role)
       VALUES (?::int, 'created', NULL, 'draft', ?::int, ?)`,
      [paper_id, session.id, session.role]
    );
