...
export async function POST(
  ...
  try {
    ...
    const parentQuestionId = parent_question_id ? parseInt(parent_question_id, 10) : null;
    const marksInt = parseInt(marks, 10) || 1;
    ...
    const existing = await query<any[]>(
      'SELECT id, parent_question_id FROM exam_paper_questions WHERE exam_paper_id = ? AND question_id = ? AND (parent_question_id IS NOT DISTINCT FROM ?::int)',
      [paperId, questionId, parentQuestionId]
    );
        ...
        if (level === 1) {
          ...
          const countQuery = await query<any[]>(
            `SELECT COUNT(*) as count 
             ...
             AND sub_question_label IS NOT NULL`,
            [paperId, parentQuestionId, level]
          );
    ...
    const result = await query<any>(
      ...
      [
        ...
        subQuestionLabel,
        nextSequenceOrder,
        marksInt || questions[0].marks,
        sectionValue,
        true,
        parentQuestionId,
        indentation_level || 0,
        can_have_sub_questions ? true : false
      ]
    );
...
export async function DELETE(
  ...
  try {
    ...
    const questionId = searchParams.get('question_id') ? parseInt(searchParams.get('question_id')!, 10) : null;
    const epqId = searchParams.get('epq_id') ? parseInt(searchParams.get('epq_id')!, 10) : null;
