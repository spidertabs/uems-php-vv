...
export async function GET(
  ...
  try {
    ...
    if (isNaN(id)) {
      return NextResponse.json({ success: false, error: 'Invalid programme ID' }, { status: 400 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    // Check if programme has associated courses (via department)
    const courses = await query<any[]>(
      'SELECT COUNT(*) as count FROM courses WHERE department_id = ?',
      [programmes[0].department_id]
    );
    ...
    await query('DELETE FROM programmes WHERE id = ?', [id]);
